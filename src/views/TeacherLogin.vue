  ...
  <div class="login-page">
    <div class="card-luxury login-card">
      <div class="login-header">
        <div class="login-logo" style="background: var(--copper-400)">师</div>
        <h1 class="text-h1" style="margin-bottom: 0.5rem">教师登录</h1>
        <p class="text-body">教师管理系统入口</p>
      </div>
      ...
      <form @submit.prevent="handleLogin">
        <div class="form-group">
          <label class="form-label">教师账号</label>
          <input
      ...
      <div style="text-align: center; margin-top: 1.5rem">
        <router-link to="/" style="color: var(--brass-500); text-decoration: none; font-size: 0.875rem">
          ← 返回学生入口
        </router-link>
      </div>
    </div>
...
const handleLogin = async () => {
  try {
    const result = await request('/api/teacher_login.php', {
      method: 'POST',
